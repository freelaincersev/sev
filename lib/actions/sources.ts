...
/**
 * Shared ingestion pipeline (add-source and save-answer): create the source
 * row, archive the original, then chunk → embed → ready.
 */
async function ingestSource({
  ...
  const storagePath = `${userId}/${projectId}/${source.id}/${sanitizeFilename(
    originalFilename,
  )}`;
...
export type SaveAnswerState = { ok?: boolean; error?: string };

/**
 * B-1: save an Ask answer into the project's memory as a `saved_answer`
 * source. It goes through the normal ingestion pipeline, so it's retrievable
 * by later questions like any other source (B-3).
 */
export async function saveAnswerToMemory(
  ...
  const intent = String(formData.get("intent") ?? "").trim() || null;
  if (!projectId || !query || !answer) return { error: "Nothing to save." };
  ...
  const title = query.length > 80 ? `${query.slice(0, 79)}…` : query;
  const savedOn = new Date().toISOString().slice(0, 10);
  const intentLine = intent ? `> Why this matters: ${intent}\n\n` : "";
  const content = `${intentLine}# ${query}\n\n${answer}\n\n---\n\n*Saved answer · generated by ${
    model || "AI"
  } · ${savedOn}. Citations [n] refer to the sources of the original answer.*`;
  ...
  const result = await ingestSource({
    ...
    content,
    intent,
    originalBlob: new Blob([content], { type: "text/markdown; charset=utf-8" }),
    originalFilename: "answer.md",
  });
